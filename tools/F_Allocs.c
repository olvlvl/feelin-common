...
/*** read arguments ********************************************************/
        ...
        if (rdargs = ReadArgs(TEMPLATE,(int32 *)(&args),NULL))
        {
            FreeArgs(rdargs);
        }
        ...
        for (pool = FeelinBase -> pools ; pool ; pool = pool -> next)
        {
            ...
            if (args[ARG_TREE]) Printf("[33mPool[0m 0x%08lx %s (%s) ",pool,(pool -> Semaphore.ss_Link.ln_Name) ? pool -> Semaphore.ss_Link.ln_Name : "Private",(pool -> Semaphore.ss_Owner) ? "locked" : "unlocked");
            ...
            if ((puddle = pool -> puddles) != NULL)
            {
                ...
                {
                    if (args[ARG_TREE]) Printf("   [33mPuddle[0m 0x%08.lx - [1mSize[0m %ld\n",puddle,puddle -> size - sizeof (struct FeelinPuddle));
